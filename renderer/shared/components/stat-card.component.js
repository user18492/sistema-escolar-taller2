// Componente reutilizable: tarjeta de indicador de las vistas Inicio (ícono + dato + ilustración).
// Uso: <stat-card icon="users" label="Usuarios activos" value="1,248"></stat-card> dentro de .stat-grid.
//   - "icon": users | rectangle-stack | academic-cap | clipboard-document-list | clipboard-document.
//   - "label" y "value": etiqueta y dato del indicador.
// Estilos en dashboard.css.
    ...
    connectedCallback() {
      ...
      this.innerHTML = `<div class="card stat-card">
        <svg class="stat-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
          <path d="${ICON_PATHS[this.getAttribute('icon')] ?? ''}" />
        </svg>
        <div class="stat-body">
          <span class="stat-label"></span>
          <span class="stat-value"></span>
        </div>
        <svg class="stat-illustration" viewBox="0 0 32 32" fill="currentColor" aria-hidden="true">
          <path d="${ILLUSTRATION_PATH}" />
        </svg>
      </div>`;
      this.querySelector('.stat-label').textContent = this.getAttribute('label');
      this.querySelector('.stat-value').textContent = this.getAttribute('value');
    }
  }
